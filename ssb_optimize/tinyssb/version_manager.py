...
from .ssb_util import mk_dir, to_hex, create_keypair, from_hex, walk, read_file, write_file
...
_trim_1 = lambda x: x[1:] if x.startswith("/") else x
_trim_2 = lambda x: x[:-1] if x.endswith("/") else x
trim_file_name = lambda x: _trim_1(_trim_2(x))
    ...
    def _file_feed_callback(self, fid: bytes) -> None:
        ...
        if front_type == PacketType.updfile:
            ...
            if file_name not in walk():
                ...
                if "/" in file_name:
                    # separate file name and directory path
