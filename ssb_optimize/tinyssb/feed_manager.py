...
is_feed_fn = lambda fn: fn.endswith(".log") and len(fn) == 68
...
class FeedManager:
    ...
    def __init__(self):
        ...
        self.keys = {}
        self._load_config()
        self.feed_dir = "_feeds"
        self.blob_dir = "_blobs"
        self._check_dirs()
    ...
    def save_config(self) -> None:
        ...
        f = open("feed_cfg.json", "w")
        f.write(string)
        f.close()

    def _load_config(self) -> None:
        if "feed_cfg.json" not in os.listdir():
            self.keys = {}
            return

        f = open("feed_cfg.json", "r")
        jsn = f.read()
        f.close()
    ...
    def __str__(self) -> str:
        ...
        """
        string_builder = []
        feed_fns = filter(is_feed_fn, os.listdir(self.feed_dir))

        for feed_fn in feed_fns:
            feed = Feed("".join([self.feed_dir, "/", feed_fn]))
    ...
    def __len__(self):
        is_fn = lambda fn: fn.endswith(".log") and len(fn) == 36
        return len(list(filter(is_fn, os.listdir(self.feed_dir))))

    def __getitem__(self, i: int) -> Feed:
        fn_filter = filter(is_feed_fn, os.listdir(self.feed_dir))
        feed_fn = None
        for _ in range(i):
            feed_fn = next(fn_filter)

        if feed_fn is None:
            key = None
        else:
            key = self._get_skey(feed_fn)
        print(feed_fn)
        return Feed("".join([self.feed_dir, "/", feed_fn]), key)
    ...
    def get_feed(self, fid: Union[memoryview, bytes, str]) -> Optional[Feed]:
        """
        ...
        """
        if type(fid) is memoryview:
            fid = bytes(fid)

        if type(fid) is bytes:
            fid = to_hex(fid)
        assert type(fid) is str

        if not fid.endswith(".log"):
            fid = "".join([fid, ".log"])

        # check is file exists
        if fid in os.listdir(self.feed_dir):
            key = self._get_skey(fid)
            return Feed("".join([self.feed_dir, "/", fid]), key)
    ...
    def create_child_feed(
        ...
        # create child feed
        child_payload = bytes(parent_pkt.fid) + parent_pkt.seq
