...
class Blob:
    ...
    def __init__(self, payload: bytes, ptr: bytes):
        ...
        self.signature = sha256(self.wire).digest()[:20]
...
class Packet:
    ...
    def _calc_mid(self) -> bytes:
        ...
        """
        return sha256(self._get_full()).digest()[:20]

    def _calc_dmx(self) -> bytes:
        """
        Calculates the demultiplexing field of the packet.
        """
        return sha256(self.block_name).digest()[:7]
...
def create_genesis_pkt(
    fid: bytes, payload: bytes, skey: bytes
) -> Packet:
    """
...
def create_apply_pkt(
    ...
    seq: Union[bytes, int],
    prev_mid: bytes,
    file_fid: Union[bytes, memoryview],
    ...
    if type(file_fid) is memoryview:
        file_fid = bytes(file_fid)
...
def create_chain(
    ...
    chain_len = ceil((content_len - 28) / 100)  # how many elements are expected

    # for creating chain later
    chain = deque([], chain_len)

    # get size as VarInt and prepend to content
    content = to_var_int(content_len) + content
