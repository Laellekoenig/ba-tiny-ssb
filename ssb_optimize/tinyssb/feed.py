...
slice_fid = lambda x: x[
    8:40
]  # for retrieving fid contained within payload of packet wire
...
class Feed:
    ...
    def __init__(self, file_name: str, skey: Optional[bytes] = None):
        ...
        # reserved = header[:12]
        self.fid = header_mv[12:44]
        self.parent_id = header_mv[44:76]
        self.parent_seq = int.from_bytes(header_mv[76:80], "big")
        self.anchor_seq = int.from_bytes(header_mv[80:84], "big")
        self.anchor_mid = header_mv[84:104]
        self.front_seq = int.from_bytes(header_mv[104:108], "big")
        self.front_mid = header_mv[108:128]

    def __str__(self) -> str:
        """
        Used for visualizing the current state of the feed in the console.
        """
        self_get_type = self.get_type  # buffering
    ...
    def __getitem__(self, seq: int) -> bytes:
        ...
        pkt_wire_mv = memoryview(pkt_wire)
        pkt_type = pkt_wire_mv[7:8]
        payload = pkt_wire_mv[8:56]
        ...
        # blob chain
        content_size, num_bytes = from_var_int(bytes(payload))

        # preallocate final array
        content = bytearray(content_size)

        # content = bytes(payload[num_bytes:-20])
        size = 28 - num_bytes
        content[0:size] = bytes(payload[num_bytes:-20])

        # "unwrap" chain
        self_get_blob = self._get_raw_blob  # buffering
        ptr = payload[-20:]
        null_ptr = b""
        while ptr != null_ptr:  # null pointer at end
            blob = self_get_blob(ptr)
            assert blob is not None, "failed to get full blob chain"

            # "append" to array
            ptr = blob[100:]
            if size + 100 > content_size:
                # cut off
                diff = content_size - size
                content[size : size + diff] = bytes(blob[:diff])
            else:
                content[size : size + 100] = bytes(blob[:100])
                size += 100
    ...
    def _get_raw_blob(self, ptr: memoryview) -> Optional[memoryview]:
        """
        Returns the content of the blob file that the given pointer is
        pointing to.
        """
        # first two bytes of hash are the name of the subdirectory
        # ab23e5g... -> _blobs/ab/23e5g...
        hex_hash = to_hex(ptr)
        file_name = "_blobs/" + hex_hash[:2] + "/" + hex_hash[2:]
    ...
    def get_want(self) -> bytes:
        ...
        """
        want_dmx = hashlib.sha256(bytes(self.fid) + b"want").digest()[:7]
